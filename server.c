...
/*
 * This function 從client端接受一個數並相乘
...
/*
 * 方法:  利用 the queue.h push function 把 connection放到 queue上，然後把mutex unlocked
 * 並設置cond_signal去提醒threads in cond_wait，connection已經到了且準備讀取。
 */
...
/*
 * 利用pthread_cond_wait()等待signal指示queue中有一個element，然後繼續把connection移出queue並return
 */
int queue_get()
{
       /*Locks the mutex*/
